...
class EnsembleCPDModel(ABC):
    ...
    def predict(self, inputs: torch.Tensor, scale: int = None,  step: int = 1, alpha: float = 1.) -> torch.Tensor:
        """Make a prediction.
    ...
    def get_min_max_predictions(self, inputs: torch.Tensor, mode: str, scale: int = None) -> torch.Tensor:
...
class CusumEnsembleCPDModel(EnsembleCPDModel):
    ...
    def __init__(
        ...
        if not self.conditional:
            assert global_sigma is not None, "Global sigma is required for non-conditional statisctics."
